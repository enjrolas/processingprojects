...
void setup()
{
  ...
  { 
   ...
   if(i==0)   
   {
    a=new level(numPoints, radius, skip,0);
   }
   else
   {
    level b=(level)levels.get(i-1);
    a=new level(numPoints, b.childRadius, skip,b.angle+angle_step/2, b.tilt+360/numLayers);
   }
...
void draw()
{
 ...
 // ellipse(width/2,height/2,a.radius*2,a.radius*2);
  ...
  do{
    ...
    if(i>=1)
    {
      poi=(myPoint)intersections.get(i-1);
    }
    else
