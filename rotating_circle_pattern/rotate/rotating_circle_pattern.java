import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class rotating_circle_pattern extends PApplet {
LinkedList levels;
...
public void draw()
{
 ...
 // line(point1.x,point1.y,point2.x,point2.y);
  line(width/2+a.radius*cos(radians(a.angle+theta)),height/2+a.radius*sin(radians(a.angle+theta)),width/2+a.radius*cos(radians(a.angle+theta+2*angle_step)),height/2+a.radius*sin(radians(a.angle+theta+2*angle_step)));
  triangle(width/2+a.radius*cos(radians(a.angle+theta)),height/2+a.radius*sin(radians(a.angle+theta)),width/2+a.childRadius*cos(radians(a.angle+theta-angle_step/2)),height/2+a.childRadius*sin(radians(a.angle+theta-angle_step/2)),width/2+a.childRadius*cos(radians(a.angle+theta+angle_step/2)),height/2+a.childRadius*sin(radians(a.angle+theta+angle_step/2)));
  theta+=angle_step;
  if(theta>360)
    theta-=360;
...
public void findIntersections()
{
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "rotating_circle_pattern" });  }}