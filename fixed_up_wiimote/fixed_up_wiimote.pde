...
void draw() {
 ...
 /*   myPoint[0]=width/2;
   ...
   for(int i=0;i<points.size()-3;i++)
  {
    ...
    fill(a.depth*60-128,0,a.depth*50-128);
    /*beginShape();
    

      vertex(a.x,a.y,a.z);
    a=(tPoint)points.get(i+1);
      vertex(a.x,a.y,a.z);
    a=(tPoint)points.get(i+2);
      vertex(a.x,a.y,a.z);
    a=(tPoint)points.get(i+3);
      vertex(a.x,a.y,a.z);
    endShape(CLOSE);  
    */
    //tPoint a=(tPoint)points.get(i);
...
void getWiiAccels()
{
  ...
  if (wiimote_file.length == 1)
  {
    ...
    int accX = Integer.parseInt(wiimote_values[0]);
    int accY = Integer.parseInt(wiimote_values[1]);
    int accZ = Integer.parseInt(wiimote_values[2]);
...
{ 
	cam.read(); 
		...
		virgin.copy(cam, 0, 0, cam.width, cam.height,  
				0, 0, virgin.width, virgin.height);
